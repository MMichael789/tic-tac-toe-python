...
def boardMove(board, letter, move):
    board[move] = letter
...
def computerMove(board, computerLetter):
    ...
    for i in range(1, 10):
        ...
        if isSpaceFree(copy, i):
            boardMove(copy, computerLetter, i)
            if winnerCheck(copy, computerLetter):
                return i

    for i in range(1, 10):
        copy = copyBoard(board)
        if isSpaceFree(copy, i):
            boardMove(copy, playerLetter, i)
...
while True:
    theBoard = [' '] * 10
    playerLetter, computerLetter = inputPlayerMarker()
    turn = firstPlayer()
    ...
    while gameIsPlaying:
        if turn == 'player':
            ...
            boardMove(theBoard, computerLetter, move)

            if winnerCheck(theBoard, computerLetter):
                displayBoard(theBoard)
